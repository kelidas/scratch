...
from cut_fibers.specimen_3D.fibers import Fibers, Specimen
from cut_fibers.specimen_3D.probability import probability_cut_nooverlaps
...
def pullout2( w, le, phi ):
    ...
    return  mean( ( d * Heaviside( p - d ) + p * Heaviside( d - p ) ), axis = 0 )
...
#    print 'cut fibers area', A_cut, 'Ac', spec._get_cross_area(), 'af', A_cut / spec._get_cross_area()*100, '%' 
    ...
    P = tau * le_cut * e ** ( f * phi_x_cut )
    print 'mean and variance of P', mean( P ), var( P )
    print 'fdjgdjcjhcjghvkv', P
    P = pullout2( w, le_cut, phi_x_cut )
    print 'mean and variance of P', P[-1]
    #print sum( P, axis=1 )
...
plt.figure( 100 )
print 'dasfhasgklashg', mean( array( P ), axis = 1 )
...
def fibers_2d_xy():
    ...
    for i in range( 0, len( sx[0] ) ):
        ...
        ax3.add_line( l )
    ax3.plot( sx, sy, 'bo' )
    ax3.plot( [ -spec.l_x / 2., spec.l_x / 2. ], [spec.l_y / 2., spec.l_y / 2.], 'k-' )
    ax3.plot( [ -spec.l_x / 2., spec.l_x / 2. ], [-spec.l_y / 2., -spec.l_y / 2.], 'k-' )
    ax3.plot( [ spec.l_x / 2., spec.l_x / 2. ], [-spec.l_y / 2., spec.l_y / 2.], 'k-' )
    ax3.plot( [ -spec.l_x / 2., -spec.l_x / 2. ], [-spec.l_y / 2., spec.l_y / 2.], 'k-' )
