...
class fiber_tt_2p(RF):
    ...
    title = Str('minimum of 5RVs')

    def __call__(self, e, x1, x2, x3, x4, x5):
        return np.minimum(np.minimum(np.minimum(np.minimum(x1, x2), x3), x4), x5)

    cython_code = '''
            q = np.minimum(np.minimum(np.minimum(np.minimum(x1, x2), x3), x4), x5)
            '''

    weave_code = '''
            q = fmin(fmin(fmin(fmin(x1,x2),x3),x4),x5);
            '''
...
def create_demo_object(fig_output_dir='fig'):
    ...
    n_int_range = np.arange(1, 40, 1).repeat(1)
