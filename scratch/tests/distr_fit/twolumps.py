...
if __name__ == '__main__':
    ...
    quickfit.add( 'beta' )
    quickfit.add('uniform')
    #quickfit.add('weibull_min')
    #quickfit.add( 'expon' )
    #quickfit.add( 'exponweib' )
    #quickfit.add( 'exponpow' )
    quickfit.add('norm')

    if opts.sample:
        data = getattr( numpy.random, opts.sample )( size=opts.size ) 
        data = getattr( numpy.random, opts.sample )( size=2132 )   
        from numpy import loadtxt, isnan
        #data = loadtxt('cf.txt').flatten()
        #data = loadtxt( 'slack.txt' ).flatten()
        #data = loadtxt( 'cfl_cuts.txt' ).flatten()
        #data = data[data >= 0]
    else:
