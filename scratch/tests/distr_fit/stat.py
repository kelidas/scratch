...
targetdist = ['norm', 'uniform', 'triang', 'beta', 'expon']#, 'lognorm'
...
#TODO: calculate correct tail probability for convolution
#my_rvs = np.loadtxt( 'data.txt', dtype='float' )
from numpy import loadtxt, isnan
my_rvs = loadtxt( 'cf.txt' ).flatten()
#my_rvs = loadtxt( 'slack.txt' ).flatten()
#my_rvs = loadtxt( 'cfl_cuts.txt' ).flatten()
my_rvs = my_rvs[my_rvs >= 0]
...
##          (repr(arg_est),loc_est,scale_est)
    ...
    plt.savefig( r'fig/%s%s%02d_%s.png' % ( prefix, ri, ii, distname ) )
##plt.show()
##plt.close()
