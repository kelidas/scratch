...
means25 = means.copy()
means25[0] = stats.sstats.spirridatpercentile(data.exog[:,0], 25)
means75 = means.copy()
means75[0] = lowinc_75per = stats.sstats.spirridatpercentile(data.exog[:,0], 75)
...
for i in range(nobs):
    quants[i] = stats.sstats.spirridatpercentile(res, p[i]*100)
mu = res.mean()
sigma = res.std()
