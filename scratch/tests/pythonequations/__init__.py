...
#    Version info: $Id: __init__.py 279 2010-11-08 14:43:54Z zunzun.com $
...
svnRevisionNumber = '$Id: __init__.py 279 2010-11-08 14:43:54Z zunzun.com $'.split()[2] # subversion keyword subtitution on checkin
...
switch(extFormFlag) // does nothing for "case 0", the non-extended form of the base equation
{
    ...
    case 15: // InverseXYWithOffset
        temp = _id[_cwo[len_cgl-1] + i] / temp + coeff[_nc];
        break;
    ...
    case 9: // X_ExponentialDecayAndOffset
        temp = temp / (coeff[_nc] * _id[_cwo[len_cgl-1] + i]) + coeff[_nc+1];
        break;
...
{for (i=0; i < _ndp; i++){try{errno = 0;''' + equation.function_cpp_code + extendedFormSwitchCaseCode + '''if (errno){temp = 1.0E300;}
#ifdef __GNUC__
...
{summation = 0.0;for(i=0; i<_ndp; i++){try{errno = 0;''' + equation.function_cpp_code + extendedFormSwitchCaseCode + '''if (errno){summation = 1.0E300;break;}
#ifdef __GNUC__
