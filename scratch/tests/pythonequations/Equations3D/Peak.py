...
class LogisticA3D(pythonequations.EquationBaseClasses.Equation3D):
    ...
    RequiresAutoGeneratedInverseForms = False
    _name ="Logistic A"
    _HTML = "z = 4a * exp(-((x-b)/c))/((1+exp(-((x-b)/c)))<sup>2</sup>) + 4d * exp(-((y-f)/g))/((1+exp(-((y-e)/f)))<sup>2</sup>)"
    coefficientDesignatorTuple = ("a", "b", "c", "d", "f", "g")
    ...
    function_cpp_code += 'temp = 4.0 * coeff[0] * exp(temp_x_sq / pow(1.0 + exp(temp_x_sq), 2.0)) + 4.0 * coeff[3] * exp(temp_y_sq / pow(1.0 + exp(temp_y_sq), 2.0));'
    ...
    def SpecificCodeCPP(self):
        s = "\ttemp = 4.0 * a * exp(-((x_in-b)/c)) / pow(1.0 + exp(-((x_in-b)/c)), 2.0) + 4.0 * d * exp(-((y_in-e)/f)) / pow(1.0 + exp(-((y_in-f)/g)), 2.0);\n"
        return s
...
class LogisticB3D(pythonequations.EquationBaseClasses.Equation3D):
    ...
    RequiresAutoGeneratedInverseForms = False
    _name ="Logistic B"
    _HTML = "z = 16a * exp((-((x-b)/c)-((y-d)/e)) / ((1+exp(-((x-b)/c)))<sup>2</sup> * (1+exp(-((y-d)/f)))<sup>2</sup>)"
    coefficientDesignatorTuple = ("a", "b", "c", "d", 'f')
    # reuse the existing C++ temp_x_sq and temp_y_sq variables.  A little touch of algebra is used for the negatives here
    function_cpp_code  = 'temp_x_sq = -1.0 * ((_id[_cwo[0]+i] - coeff[1]) / coeff[2]);\n'
    ...
    def SpecificCodeCPP(self):
        s = "\ttemp = 16.0 * a * exp(-((x_in-b)/c)-((y_in-d)/e)) / (pow(1.0 + exp(-((x_in-b)/c)), 2.0) * pow(1.0 + exp(-((y_in-d)/f)), 2.0));\n"
        return s
