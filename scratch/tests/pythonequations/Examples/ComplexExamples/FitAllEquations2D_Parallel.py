...
if __name__ == "__main__":
    ...
    ##############################################
    number_of_cpu_stats.spirrids = multiprocessing.cpu_count() # hit the server hard using all CPU stats.spirrids at the same time
    pool = multiprocessing.Pool(processes=number_of_cpu_stats.spirrids)
    poolItems = []
    for i in range(number_of_cpu_stats.spirrids):
        poolItems.append(pool.apply_async(IterativelyFitData, (number_of_cpu_stats.spirrids, i,))) # divides the equations among the processes
    pool.close()
    pool.join()
    for i in range(number_of_cpu_stats.spirrids):
