...
from traits.api import HasTraits, Float, Property, cached_property, \
    Event, Array, Instance, Bool, List
...
class Distribution(HasTraits):
    ...
    shape = List([.5, 2.], auto_set = False, enter_set = True, params = True)
    ...
    def changes(self):
        ...
        self.remove_listeners()
        self.new_values = array([self.shape[0], self.loc, self.scale, self.mean,
        ...
        if len(indexing) > 0 and indexing[0] < 3:
            ...
            pass
        self.old_values = array([self.shape[0], self.loc, self.scale, self.mean,
                            self.variance, self.skewness, self.kurtosis])
        self.add_listeners()
        self.changed = True
...
if __name__ == '__main__':
    distr = Distribution(scipy.stats.beta)
