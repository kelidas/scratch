...
try :
  ...
  aCollection.add(Distribution(Uniform(-1.0, 2.0)))
  aCollection.add(Distribution(Gamma(2.0, 2.0, 0.0)))
  ...
  distribution = Distribution(ComposedDistribution(aCollection, Copula(NormalCopula(RCopula))))
  # Test for sampling
  size = 10000
  ...
  coll[0] = Distribution(Uniform(-1.0 + eps, 2.0))
  coll[1] = aCollection[1]
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  coll[0] = Distribution(Uniform(-1.0 - eps, 2.0))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  dTdp = (left(point) - right(point)) * factor
  gradient[0, 0] = dTdp[0]
  gradient[0, 1] = dTdp[1]
  # dT/dp1
  coll = DistributionCollection(dim)
  coll[0] = Distribution(Uniform(-1.0, 2.0 + eps))
  coll[1] = aCollection[1]
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  coll[0] = Distribution(Uniform(-1.0, 2.0 - eps))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  ...
  coll[1] = Distribution(Gamma(2.0 + eps, 2.0, 0.0))
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  coll[1] = Distribution(Gamma(2.0 - eps, 2.0, 0.0))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  dTdp = (left(point) - right(point)) * factor
  ...
  coll = DistributionCollection(dim)
  coll[0] = aCollection[0]
  coll[1] = Distribution(Gamma(2.0, 2.0 + eps, 0.0))
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  coll[1] = Distribution(Gamma(2.0, 2.0 - eps, 0.0))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  dTdp = (left(point) - right(point)) * factor
  ...
  coll = DistributionCollection(dim)
  coll[0] = aCollection[0]
  coll[1] = Distribution(Gamma(2.0, 2.0, 0.0 + eps))
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  coll[1] = Distribution(Gamma(2.0, 2.0, 0.0 - eps))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getIsoProbabilisticTransformation()
  dTdp = (left(point) - right(point)) * factor
  ...
  coll[0] = Distribution(Uniform(-1.0 + eps, 2.0))
  coll[1] = aCollection[1]
  left = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getInverseIsoProbabilisticTransformation()
  coll[0] = Distribution(Uniform(-1.0 - eps, 2.0))
  right = ComposedDistribution(coll, Copula(NormalCopula(RCopula))).getInverseIsoProbabilisticTransformation()
  dTdp = (left(point) - right(point)) * factor
  gradient[0, 0] = dTdp[0]
  gradient[0, 1] = dTdp[1]
  # dT/dp1
