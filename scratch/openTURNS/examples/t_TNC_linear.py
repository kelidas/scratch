...
try :
  ...
  myAlgorithm.run()
  print "minimizer = ", printNumericalPoint(BoundConstrainedAlgorithm(myAlgorithm).getResult().getOptimizer(), 4), " value=", BoundConstrainedAlgorithm(myAlgorithm).getResult().getOptimalValue()
  myAlgorithm = TNC(specific, levelFunction, bounds, startingPoint, BoundConstrainedAlgorithmImplementationResult.MAXIMIZATION)
  myAlgorithm.run()
  print "maximizer = ", printNumericalPoint(BoundConstrainedAlgorithm(myAlgorithm).getResult().getOptimizer(), 4), " value=", BoundConstrainedAlgorithm(myAlgorithm).getResult().getOptimalValue()
