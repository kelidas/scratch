...
def matplotlib_plot( graph, fignum ):
    '''
    ...
    data = ''
    for cell in a:
        if re.search( r'values=\[\S+\]' , cell ) != None:
            data += re.search( r'\[\S+\]' , cell ).group() + ','
    data = np.array( eval( data[:-2] ) )
    plt.figure( fignum )
    #plt.ion()
    plt.minorticks_on()
    plt.grid( which = 'both' )
