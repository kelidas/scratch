...
class FFnetHelp(HasTraits):
    ...
    def _help_string_update(self):
        ...
        self.help_string += '----------------------------------------------'
        self.help_string += getattr(ffnet.ffnet,
                                    self.training_algorithm).__doc__ + '\n'
...
class FFnet(HasTraits):
    ...
    def _create_net_fired(self):
        ...
        #self.net.weights[:] = 1
        #self.net.renormalize = False
        print 'weights =', self.net.weights
    ...
    traits_view = View(
        Item('mode', style='custom', editor=EnumEditor(
            values=mode.values, cols=4)),
        HGroup(
        ...
        Tabbed(
            ...
            Group(
                ...
                Item(
                    'train_config', springy=True, editor=CodeEditor(show_line_numbers=False)),
                UItem('help'),
                '_',
                Group(
                    UItem('ffnet_train'),
...
class FFnetView(HasTraits):
    ...
    def _load_data_fired(self):
        ...
        if self.input_file == '' or self.target_file == '':
            ...
            self.ffnet.target = np.loadtxt(
                self.target_file, skiprows=1, ndmin=2)
