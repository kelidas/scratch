...
class RV2(RF):
    ...
    title = Str('minimum of 3RVs')

    def __call__(self, e, x1, x2, x3):
        return np.minimum(np.minimum(x1, x2), x3)

    cython_code = '''
                q = np.minimum(np.minimum(x1, x2), x3)
            '''

    weave_code = '''{
                  q = fmin(fmin(x1,x2),x3);
                  //printf("%100f\\n",q);
            }
            '''
...
def create_demo_object(fig_output_dir='fig'):
    ...
    powers = np.linspace(1, math.log(60, 10), 500)
